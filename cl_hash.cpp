...
unsigned int gpu_hashing(uint256<256>* target, unsigned char* header, bool verbose) {
    ...
    unsigned int size = 0xFFFFFFFF - (local_item_size - 1); // to make multiple of loca_item_size
    size_t global_item_size = size; // Process the entire lists

    const char kernelDir[] = "bitcoin_hash_header.cl";
...
//    for(int i = 0; i < 8; i++) {
//        std::cout << "target " << i << ": " << Crypto::decToHex(target_int[i]) << std::endl;
//    }
